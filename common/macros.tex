...
\newcommand{\SourceVariantOf}[4]{}         % target label, author, book/source, relation kind
\newcommand{\LRAWorkedExampleFor}[1]{}     % comma-separated formal labels this worked example illustrates
\newcommand{\LRAWorkedExampleUses}[1]{}    % comma-separated formal labels used in the worked example
\newcommand{\LRAWorkedExampleTags}[1]{}    % comma-separated explorer tags for worked-example search/filtering
...
%   LRA_EDITION=print latexmk -lualatex volume-i.tex
%   LRA_PRINT_EDITION=1 latexmk -lualatex volume-i.tex
% or by command-line TeX injection:
%   lualatex "\def\LRAPrintEditionRequested{1}\input{volume-i.tex}"
%
...
\ifdefined\directlua
  \directlua{
    ...
    if edition and edition:lower() == "print" then
      tex.sprint("\\global\\LRAPrintEditiontrue")
    elseif v and v ~= "" and v ~= "0" and v:lower() ~= "false" and v:lower() ~= "no" then
      tex.sprint("\\global\\LRAPrintEditiontrue")
    end
  }
\fi
...
\long\def\LRAExcludeFromPrintEditionBegin#1\LRAExcludeFromPrintEditionEnd{%
  \ifLRAPrintEdition\else#1\fi
}
\let\LRAExcludeFromPrintEditionEnd\relax
