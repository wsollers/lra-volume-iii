...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:interval-partition}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{IntervalPartition}{pending}
...
\[
\neg\mathsf{Partition}(P,I) \Longleftrightarrow
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:tagged-partition}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{TaggedPartition}{pending}
...
\begin{remark*}[Predicate reading]
\[
\mathsf{TaggedPartition}(\dot{\mathcal{P}},P,[a,b])
\coloneqq
\Bigl(\mathsf{Partition}(P,[a,b]) \land \dot{\mathcal{P}}=\{([x_{i-1},x_i],t_i)\}_{i=1}^n \land \forall i\;(t_i \in [x_{i-1},x_i])\Bigr),
\]
where $\mathsf{Partition}(P,[a,b])$ abbreviates $a=x_0<\cdots<x_n=b$ for $P=\{[x_{i-1},x_i]\}_{i=1}^n$.
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
\neg\mathsf{TaggedPartition}(\dot{\mathcal{P}},P,[a,b])
\Longleftrightarrow
(x_0\ne a)\lor(x_n\ne b)\lor
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:gauge}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{IsGauge}{pending}
...
\begin{definitionbox}{Definition ($\delta$-Fine Partition)}
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:delta-fine-partition}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{IsDeltaFine}{pending}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{IsDeltaFine}(\dot{\mathcal{P}},\delta,I) \coloneqq \forall i \in \{1,\dots,n\}\;\forall x \in [x_{i-1},x_i]\;( |x-t_i|<\delta(t_i)).
\]
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
\neg \operatorname{IsDeltaFine}(\dot{\mathcal{P}},\delta,I) \Longleftrightarrow \exists i \in \{1,\dots,n\}\;\exists x \in [x_{i-1},x_i]\;( |x-t_i|\ge \delta(t_i)).
\]
\end{remark*}
...
\end{lemma}
\end{lemmabox}
\LeanFormalizes{lem:every-point-covered-by-tag}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{EveryPointCoveredByTag}{pending}
...
\begin{theorembox}{Theorem (Cousin's Theorem)}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:cousins-theorem}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{CousinsTheorem}{pending}
...
&\mathsf{TaggedPartition}(\dot{\mathcal{P}},P,[a,b])\\
&\land
\operatorname{IsDeltaFine}(\dot{\mathcal{P}},\delta,I)
\Bigr).
\end{aligned}
\]
\end{remark*}
...
\mathsf{TaggedPartition}(\dot{\mathcal{P}},P,[a,b])
\Rightarrow
\neg\operatorname{IsDeltaFine}(\dot{\mathcal{P}},\delta,I)
\Bigr).
\]
\end{remark*}
...
\mathsf{TaggedPartition}(\dot{\mathcal{P}},P,[a,b])}_{\text{all tagged partitions}}
\quad\Rightarrow\quad
\underbrace{\neg\operatorname{IsDeltaFine}(\dot{\mathcal{P}},\delta,I)}_{\text{some interval escapes its gauge ball}}.
\]
...
% Lean-aligned addition: thm:common-refinement-exists
\begin{theorem}[Common Refinement Exists]
\label{thm:common-refinement-exists}
Any two interval partitions of the same compact interval $[a,b]$ have a common refinement.

\smallskip
\noindent
\hyperref[prf:common-refinement-exists]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:common-refinement-exists}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{CommonRefinementExists}{pending}
