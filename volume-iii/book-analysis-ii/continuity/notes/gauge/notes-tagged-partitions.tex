...
\[
m=\|P\| \coloneqq m=\max_{1\le i\le n}(x_i - x_{i-1}).
\]
\end{remark*}
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:refinement-of-partition}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{IsRefinementOf}{pending}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{IsRefinement}(Q,P) \coloneqq P \subseteq Q.
\]
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
\neg \operatorname{IsRefinement}(Q,P) \Longleftrightarrow \exists x \in P\; (x \notin Q).
\]
\end{remark*}
...
Failure occurs precisely when some point listed in P fails to appear in Q, meaning Q omits at least one of the partition points it was supposed to inherit from P.
  ...
  \item[\(\operatorname{IsRefinement}.\)]
  The displayed obstruction is the corresponding failure mode.
  \[
\exists x\;(x\in P \land x\notin Q).
  \]
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:common-refinement}{lra-lean}{LRA.VolumeIII.Analysis.Continuity.Gauge}{HasCommonRefinement}{pending}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{IsCommonRefinement}(R,P,Q) \coloneqq (R = P \cup Q).
\]
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
\neg \operatorname{IsCommonRefinement}(R,P,Q) \Longleftrightarrow R \ne P \cup Q.
\]
\end{remark*}
...
The claim that a partition $R$ is the common refinement of $P$ and $Q$ fails exactly when $R$ either contains a node not present in $P \cup Q$ or omits a node that $P \cup Q$ contributes. These are the only two structural ways in which $R$ can differ from $P \cup Q$.
  ...
  \item[\(\operatorname{IsCommonRefinement}.\)]
  The displayed obstruction is the corresponding failure mode.
  \[
\begin{aligned}
&a<b\land P,Q\subseteq [a,b]\land R\subseteq [a,b]\\
