...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:chain-rule}{lra-lean}{LRA.VolumeIII.Analysis.Differentiation.ChainRule}{ChainRule}{pending}

\begin{remark*}[Standard quantified statement]
\[
  \operatorname{IsDifferentiable}(f,c,\mathbb{R},\mathbb{R})
  \land
  \operatorname{IsDifferentiable}(g,f(c),\mathbb{R},\mathbb{R})
  \Longrightarrow
  (g\circ f)'(c)=g'(f(c))f'(c).
\]
\end{remark*}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:leibniz-rule}{lra-lean}{LRA.VolumeIII.Analysis.Differentiation.ChainRule}{LeibnizRule}{pending}
...
% Lean-aligned addition: thm:faa-di-bruno-second-order
\begin{theorem}[Second-Order Faa di Bruno Formula]
\label{thm:faa-di-bruno-second-order}
If $f$ and $g$ are twice differentiable where required, then
\[
...
\smallskip
\noindent
\hyperref[prf:faa-di-bruno-second-order]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:faa-di-bruno-second-order}{lra-lean}{LRA.VolumeIII.Analysis.Differentiation.ChainRule}{FaaDiBrunoSecondOrder}{pending}
