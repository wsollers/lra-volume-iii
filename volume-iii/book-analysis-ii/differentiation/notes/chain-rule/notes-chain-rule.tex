...
\begin{remark*}[Standard quantified statement]
\[
  \operatorname{Diff}^{n}(f,x)\land\operatorname{Diff}^{n}(g,x)
  \Longrightarrow
  (fg)^{(n)}(x)
  =
  \sum_{k=0}^{n}\binom{n}{k}f^{(k)}(x)g^{(n-k)}(x).
...
\begin{remark*}[Standard quantified statement]
\[
  \operatorname{Diff}^{n}(g,x)\land\operatorname{Diff}^{n}(f,g(x))
  \Longrightarrow
  (f\circ g)^{(n)}(x)
  =
  \sum_{\sum mk_m=n}
...
\begin{remark*}[Cross-volume callback]
...
\begin{definitionbox}{Example (Faa di Bruno at \(n=2\))}
...
\begin{definitionbox}{Example (Faa di Bruno at \(n=3\))}
