% !TEX root = ../../../../../volume-iii-bounds-sequences-and-series.tex
\newpage
\phantomsection
\label{prf:constant-comparison-sequence-limits}
\LRAProofFor{thm:constant-comparison-sequence-limits}

\begin{remark*}[Return]
\hyperref[thm:constant-comparison-sequence-limits]{Return to Theorem}
\end{remark*}

\begin{theorem*}[Constant Comparison for Sequence Limits]
Let $(x_n)$ be a real sequence, and let $A,B\in\mathbb{R}$. Suppose
that $x_n\to A$.
\begin{enumerate}[label=(\alph*)]
  \item If there exists $N\in\mathbb{N}$ such that $x_n\le B$ for every
  $n\ge N$, then $A\le B$.
  \item If there exists $N\in\mathbb{N}$ such that $x_n<B$ for every
  $n\ge N$, then $A\le B$.
  \item If there exists $N\in\mathbb{N}$ such that $x_n\ge B$ for every
  $n\ge N$, then $A\ge B$.
  \item If there exists $N\in\mathbb{N}$ such that $x_n>B$ for every
  $n\ge N$, then $A\ge B$.
\end{enumerate}

\smallskip
\noindent
\end{theorem*}

\begin{proof}[Professional Standard Proof]
\LRAProofBodyStart
TODO: professional standard proof for constant-comparison-for-sequence-limits.
\end{proof}

\begin{proof}[Detailed Learning Proof]
\LRAProofBodyStart
TODO: detailed learning proof for constant-comparison-for-sequence-limits.
\end{proof}

\begin{remark*}[Proof structure]
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item TODO: list mathematical dependencies.
\end{itemize}
\end{dependencies}

\clearpage
