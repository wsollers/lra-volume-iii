...
this with a punctured-neighborhood condition and asserts
...
\begin{aligned}
  ...
  \operatorname{PuncturedNeighborhood}(U^\times_\delta,U_\delta,c,\delta,\mathbb R)
  &\quad\text{and}\quad
  \operatorname{Within}(x,U^\times_\delta,\mathbb R)
  &\Longleftrightarrow 0<|x-c|<\delta,\\
  \operatorname{Ball}(B_\varepsilon(L),L,\varepsilon,\mathbb R)
  &\quad\text{and}\quad
  \operatorname{Within}(y,B_\varepsilon(L),\mathbb R)
  ...
  \Bigl(
    ...
    \bigl(
      ...
      [
        \operatorname{Within}(x,U^\times_\delta,\mathbb R)
        \Rightarrow
        \operatorname{Within}(f(x),B_\varepsilon(L),\mathbb R)
      ]
    \bigr)
  \Bigr).
\end{aligned}
  ...
  \Bigl(
    ...
    \bigl(
      ...
      [
        \operatorname{Within}(x,U^\times_\delta,\mathbb R)
        \wedge
        \neg\,\operatorname{Within}(f(x),B_{\varepsilon_0}(L),\mathbb R)
      ]
    \bigr)
  \Bigr)\\
  &\Longleftrightarrow
...
condition is punctured (a punctured-neighborhood condition, not
...
\begin{definitionbox}{Definition (Neighbourhood Form of Limit)}
\begin{definition}[Neighbourhood Form of Limit]
\label{def:limit-function-neighbourhood}
Let $f : A \to \mathbb{R}$ and let $c$ be a cluster point of $A$.
The limit $\lim_{x\to c}f(x)=L$ in the \textbf{neighbourhood
sense} holds if for every $\varepsilon$-neighbourhood
$V_\varepsilon(L)$ there exists a deleted $\delta$-neighbourhood
$V_\delta^*(c)$ such that
\[
...
\begin{remark*}[Standard quantified statement]
\[
  \forall\varepsilon>0\;\exists\delta>0\;\forall x\in V_\delta^*(c)\cap A\;
  f(x)\in V_\varepsilon(L).
\]
\end{remark*}

\begin{remark*}[Interpretation]
The neighbourhood form packages the same local limit condition in
terms of sets around the input point and output value. The domain is
still restricted to $A$, and the neighbourhood of $c$ is deleted so
...
\Bigl[\forall\varepsilon>0\;\exists\delta>0\;\forall x\in V_\delta^*(c)\cap A\;
  f(x)\in V_\varepsilon(L)\Bigr].
\end{aligned}
\]
\end{remark*}
...
V_\varepsilon(L)$ is exactly $\operatorname{Within}(f(x),B_\varepsilon(L),
\mathbb R)$. The neighbourhood form makes the geometric content
...
\begin{itemize}
  \item \hyperref[def:limit-function]{Limit of a Function}
  \item \hyperref[def:limit-function-neighbourhood]{Neighbourhood Form of Limit}
  \item \hyperref[def:epsilon-neighbourhood]{$\varepsilon$-Neighbourhood}
  \item \hyperref[def:deleted-epsilon-neighbourhood]{Deleted $\varepsilon$-Neighbourhood}
\end{itemize}
\end{dependencies}
...
\begin{itemize}
  ...
  \item \hyperref[def:limit-function-neighbourhood]{Neighbourhood Form of Limit}
  \item \hyperref[def:limit-function-sequential]{Sequential Definition of Limit}
  \item \hyperref[prop:limit-neighbourhood-equiv]{Neighbourhood Form of the Limit}
