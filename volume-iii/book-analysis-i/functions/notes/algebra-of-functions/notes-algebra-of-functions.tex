% !TEX root = ../../../../../volume-iii-bounds-sequences-and-series.tex
% =========================================================
% Algebra of Functions --- Notes
% Chapter: functions
% Section: algebra-of-functions
% Volume III $\cdot$ Analysis
% =========================================================

\subsection*{Algebra of Functions}
\label{sec:algebra-of-functions}
\begin{remark*}[Interpretation]
Every open interval $(a,b)$ consists entirely of cluster points;
$[a,b]$ is closed and bounded. Natural domains for functions are
typically intervals or unions of intervals.
\end{remark*}

% ---------------------------------------------------------
% Proposition --- Composition of Injections
% ---------------------------------------------------------

\begin{propositionbox}{Proposition}
\begin{proposition}[Composition of Injections Is Injective]
\label{prop:composition-injective}
Let $f:A\to B$ and let $g:B\to C$. If $f$ is injective and $g$ is
injective, then $g\circ f:A\to C$ is injective.

\smallskip\noindent
\hyperref[prf:composition-injective]{\textit{Go to proof.}}
\end{proposition}
\end{propositionbox}
\LeanFormalizes{prop:composition-injective}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{CompositionInjective}{pending}

\begin{remark*}[Standard quantified statement]
\[
\begin{aligned}
&\Bigl(\forall a_1,a_2\in A\;(f(a_1)=f(a_2)\Rightarrow a_1=a_2)\Bigr)
\wedge
\Bigl(\forall b_1,b_2\in B\;(g(b_1)=g(b_2)\Rightarrow b_1=b_2)\Bigr) \\
&\quad\Rightarrow
\forall a_1,a_2\in A\;(g(f(a_1))=g(f(a_2))\Rightarrow a_1=a_2).
\end{aligned}
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  f\colon A\to B,\qquad
  g\colon B\to C,\qquad
  g\circ f\colon A\to C,\qquad
  \operatorname{Injective}(f,A,B)\wedge\operatorname{Injective}(g,B,C)
  \;\Longrightarrow\;
  \operatorname{Injective}(g\circ f,A,C).
\]
\end{remark*}

\begin{remark*}[Negated quantified statement]
\[
  \exists a_1,a_2\in A\;\bigl(a_1\neq a_2\;\wedge\;g(f(a_1))=g(f(a_2))\bigr).
\]
\end{remark*}

\begin{remark*}[Negation predicate reading]
\[
  \neg\,\operatorname{Injective}(g\circ f,A,C).
\]
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.]
  The negated statement isolates the obstruction to the asserted condition.

  \item[\(\operatorname{Injective}.\)]
  The displayed obstruction is the corresponding failure mode.
  \[
  \exists a_1,a_2\in A\;\bigl(a_1\neq a_2\;\wedge\;g(f(a_1))=g(f(a_2))\bigr).
\]
  \[
  \neg\,\operatorname{Injective}(g\circ f,A,C).
\]
\end{description}
\end{remark*}



\begin{remark*}[Interpretation]
Injectivity survives composition: equality after $g\circ f$ can be
pulled back first through $g$ and then through $f$.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:composition]{Composition}
  \item \hyperref[def:function]{Function}
\end{itemize}
\end{dependencies}
% ---------------------------------------------------------
% Proposition --- Composition of Surjections
% ---------------------------------------------------------

\begin{propositionbox}{Proposition}
\begin{proposition}[Composition of Surjections Is Surjective]
\label{prop:composition-surjective}
Let \(f:A\to B\) and let \(g:B\to C\). If \(f\) is surjective and \(g\)
is surjective, then \(g\circ f:A\to C\) is surjective.

\smallskip\noindent
\hyperref[prf:composition-surjective]{\textit{Go to proof.}}
\end{proposition}
\end{propositionbox}
\LeanFormalizes{prop:composition-surjective}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{CompositionSurjective}{pending}

\begin{remark*}[Standard quantified statement]
\[
  \operatorname{Surjective}(f)\wedge\operatorname{Surjective}(g)
  \;\Rightarrow\;
  \forall c\in C,\;\exists a\in A: g(f(a))=c.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  g\circ f\colon A\to C,\qquad
  \operatorname{Surjective}(g\circ f, A, C)
  \;\colonequiv\;
  \forall c\in C,\;\exists a\in A: (g\circ f)(a)=c.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Surjectivity survives composition because every target value first has a
preimage under $g$, and that preimage in turn has a preimage under $f$.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:composition]{Composition}
  \item \hyperref[def:function]{Function}
\end{itemize}
\end{dependencies}
% ---------------------------------------------------------
% Corollary --- Composition of Bijections
% ---------------------------------------------------------

\begin{corollarybox}{Corollary}
\begin{corollary}[Composition of Bijections Is Bijective]
\label{cor:composition-bijective}
Let $f:A\to B$ and let $g:B\to C$. If $f$ is bijective and $g$ is
bijective, then $g\circ f:A\to C$ is bijective.

\smallskip\noindent
\hyperref[prf:composition-bijective]{\textit{Go to proof.}}
\end{corollary}
\end{corollarybox}
\LeanFormalizes{cor:composition-bijective}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{CompositionBijective}{pending}

\begin{remark*}[Standard quantified statement]
\[
\begin{aligned}
&\Bigl[
  \bigl(\forall a_1,a_2\in A\;(f(a_1)=f(a_2)\Rightarrow a_1=a_2)\bigr)
  \wedge
  \bigl(\forall b\in B\;\exists a\in A\;(f(a)=b)\bigr)
 \Bigr] \\
&\quad\wedge
 \Bigl[
  \bigl(\forall b_1,b_2\in B\;(g(b_1)=g(b_2)\Rightarrow b_1=b_2)\bigr)
  \wedge
  \bigl(\forall c\in C\;\exists b\in B\;(g(b)=c)\bigr)
 \Bigr] \\
&\quad\Rightarrow
 \Bigl[
  \bigl(\forall a_1,a_2\in A\;(g(f(a_1))=g(f(a_2))\Rightarrow a_1=a_2)\bigr)
  \wedge
  \bigl(\forall c\in C\;\exists a\in A\;(g(f(a))=c)\bigr)
 \Bigr].
\end{aligned}
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  f\colon A\to B,\qquad
  g\colon B\to C,\qquad
  g\circ f\colon A\to C,\qquad
  \operatorname{Bijective}(f,A,B)\wedge\operatorname{Bijective}(g,B,C)
  \;\Longrightarrow\;
  \operatorname{Bijective}(g\circ f,A,C).
\]
\end{remark*}

\begin{remark*}[Negated quantified statement]
\[
\begin{aligned}
&\exists a_1,a_2\in A\;\bigl(a_1\neq a_2\;\wedge\;g(f(a_1))=g(f(a_2))\bigr) \\
&\quad\vee\;
  \exists c\in C\;\forall a\in A\;\bigl(g(f(a))\neq c\bigr).
\end{aligned}
\]
\end{remark*}

\begin{remark*}[Negation predicate reading]
\[
  \neg\,\operatorname{Bijective}(g\circ f,A,C).
\]
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.]
  The negated statement isolates the obstruction to the asserted condition.

  \item[\(\operatorname{Bijective}.\)]
  The displayed obstruction is the corresponding failure mode.
  \[
\begin{aligned}
&\exists a_1,a_2\in A\;\bigl(a_1\neq a_2\;\wedge\;g(f(a_1))=g(f(a_2))\bigr) \\
&\quad\vee\;
  \exists c\in C\;\forall a\in A\;\bigl(g(f(a))\neq c\bigr).
\end{aligned}
\]
  \[
  \neg\,\operatorname{Bijective}(g\circ f,A,C).
\]
\end{description}
\end{remark*}



\begin{remark*}[Interpretation]
A bijection is an injective and surjective function. Since both of those
properties are preserved by composition, bijectivity is preserved too.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:composition]{Composition}
  \item \hyperref[def:function]{Function}
\end{itemize}
\end{dependencies}
% ---------------------------------------------------------
% Proposition --- Inverse of Bijection
% ---------------------------------------------------------

\begin{propositionbox}{Proposition}
\begin{proposition}[Inverse of a Bijection Is a Bijection]
\label{prop:inverse-bijection}
If \(f:A\to B\) is bijective, then \(f^{-1}:B\to A\) is bijective.

\smallskip\noindent
\hyperref[prf:inverse-bijection]{\textit{Go to proof.}}
\end{proposition}
\end{propositionbox}
\LeanFormalizes{prop:inverse-bijection}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{InverseBijection}{pending}

\begin{remark*}[Standard quantified statement]
\[
  \operatorname{Bijective}(f)
  \;\Rightarrow\;
  \operatorname{InverseFunction}(f, f^{-1}),\;
  f^{-1}\circ f=\mathrm{id}_A,\;
  f\circ f^{-1}=\mathrm{id}_B.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
  f\colon A\to B,\qquad
  f^{-1}\colon B\to A,\qquad
  \operatorname{InverseFunction}(f^{-1}, f, A, B)
  \;\colonequiv\;
  f^{-1}\circ f = \mathrm{id}_A \;\wedge\; f\circ f^{-1}=\mathrm{id}_B.
\]
\end{remark*}

\begin{remark*}[Interpretation]
A bijection has an inverse function because each output is hit by exactly one
input. The inverse reverses the original correspondence and composes back to
the identity on both sides.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:composition]{Composition}
  \item \hyperref[def:function]{Function}
\end{itemize}
\end{dependencies}
% ---------------------------------------------------------
% Proposition --- Preimage Distributes
% ---------------------------------------------------------

\begin{propositionbox}{Proposition}
\begin{proposition}[Preimage Distributes over Union and Intersection]
\label{prop:preimage-union-intersection}
Let $f:A\to B$, $S,T\subseteq B$. Then:
\begin{enumerate}[label=(\roman*)]
  \item $f^{-1}(S\cup T)=f^{-1}(S)\cup f^{-1}(T)$.
  \item $f^{-1}(S\cap T)=f^{-1}(S)\cap f^{-1}(T)$.
  \item $f^{-1}(S^c)=(f^{-1}(S))^c$.
\end{enumerate}

\smallskip\noindent
\hyperref[prf:preimage-union-intersection]{\textit{Go to proof.}}
\end{proposition}
\end{propositionbox}
\LeanFormalizes{prop:preimage-union-intersection}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{PreimageUnionIntersection}{pending}

\begin{remark*}[Standard quantified statement]
\[
\begin{aligned}
(i)\quad &x\in f^{-1}(S\cup T)\iff f(x)\in S\vee f(x)\in T
         \iff x\in f^{-1}(S)\cup f^{-1}(T). \\
(ii)\quad &x\in f^{-1}(S\cap T)\iff f(x)\in S\wedge f(x)\in T
          \iff x\in f^{-1}(S)\cap f^{-1}(T).
\end{aligned}
\]
\end{remark*}

\begin{remark*}[Negated quantified statement]
\[
  x\notin f^{-1}(S)
  \;\iff\; f(x)\notin S
  \;\iff\; x\in f^{-1}(S^c).
\]
\end{remark*}

\begin{remark*}[Negation predicate reading]
\[
  x\notin f^{-1}(S)
  \;\iff\; f(x)\notin S
  \;\iff\; x\in f^{-1}(S^c).
\]
\end{remark*}

\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.]
  The negated statement isolates the obstruction to the asserted condition.

  \item[\exists-condition.]
  The displayed obstruction is the corresponding failure mode.
  \[
  x\notin f^{-1}(S)
  \;\iff\; f(x)\notin S
  \;\iff\; x\in f^{-1}(S^c).
\]
  \[
  x\notin f^{-1}(S)
  \;\iff\; f(x)\notin S
  \;\iff\; x\in f^{-1}(S^c).
\]
\end{description}
\end{remark*}



\begin{remark*}[Interpretation]
Preimage is a complete lattice homomorphism: it preserves all set
operations including complement. Contrast with image: $f(A\cap B)
\subseteq f(A)\cap f(B)$ with strict containment possible when $f$
is not injective. Preimage identities are load-bearing in topology
(preimages of open sets are open) and measure theory (preimages of
measurable sets are measurable).
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:function]{Function}
\end{itemize}
\end{dependencies}

% Lean-aligned additions inserted for Volume III Lean-to-TeX handoff.
% Lean-aligned addition: def:is-injective-on
\begin{definition}[Injective on a Set]
\label{def:is-injective-on}
A function $f:A\to B$ is \emph{injective on $A$} if $x,y\in A$ and $f(x)=f(y)$ imply $x=y$.
\end{definition}
\LeanFormalizes{def:is-injective-on}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{IsInjectiveOn}{pending}

\begin{remark*}[Standard quantified statement]
A function $f:A\to B$ is \emph{injective on $A$} if $x,y\in A$ and $f(x)=f(y)$ imply $x=y$.
\end{remark*}

\begin{remark*}[Interpretation]
This formal object records a Lean-aligned addition in the local language of this section.
\end{remark*}

\begin{dependencies}
\NoLocalDependencies
\end{dependencies}
% Lean-aligned addition: def:is-surjective-on
\begin{definition}[Surjective from One Set onto Another]
\label{def:is-surjective-on}
A function $f:A\to B$ is \emph{surjective from $A$ onto $B$} if for every $b\in B$ there exists $a\in A$ such that $f(a)=b$.
\end{definition}
\LeanFormalizes{def:is-surjective-on}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{IsSurjectiveOn}{pending}

\begin{remark*}[Standard quantified statement]
A function $f:A\to B$ is \emph{surjective from $A$ onto $B$} if for every $b\in B$ there exists $a\in A$ such that $f(a)=b$.
\end{remark*}

\begin{remark*}[Interpretation]
This formal object records a Lean-aligned addition in the local language of this section.
\end{remark*}

\begin{dependencies}
\NoLocalDependencies
\end{dependencies}
% Lean-aligned addition: def:is-bijective-on
\begin{definition}[Bijective between Sets]
\label{def:is-bijective-on}
A function $f:A\to B$ is \emph{bijective from $A$ to $B$} if it is injective on $A$ and surjective from $A$ onto $B$.
\end{definition}
\LeanFormalizes{def:is-bijective-on}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{IsBijectiveOn}{pending}

\begin{remark*}[Standard quantified statement]
A function $f:A\to B$ is \emph{bijective from $A$ to $B$} if it is injective on $A$ and surjective from $A$ onto $B$.
\end{remark*}

\begin{remark*}[Interpretation]
This formal object records a Lean-aligned addition in the local language of this section.
\end{remark*}

\begin{dependencies}
\NoLocalDependencies
\end{dependencies}
% Lean-aligned addition: def:is-inverse-function-of
\begin{definition}[Inverse Function Relation]
\label{def:is-inverse-function-of}
A function $g:B\to A$ is an \emph{inverse function} of $f:A\to B$ if $g(f(a))=a$ for every $a\in A$ and $f(g(b))=b$ for every $b\in B$.
\end{definition}
\LeanFormalizes{def:is-inverse-function-of}{lra-lean}{LRA.VolumeIII.Analysis.Functions.AlgebraOfFunctions}{IsInverseFunctionOf}{pending}

\begin{remark*}[Standard quantified statement]
A function $g:B\to A$ is an \emph{inverse function} of $f:A\to B$ if $g(f(a))=a$ for every $a\in A$ and $f(g(b))=b$ for every $b\in B$.
\end{remark*}

\begin{remark*}[Interpretation]
This formal object records a Lean-aligned addition in the local language of this section.
\end{remark*}

\begin{dependencies}
\NoLocalDependencies
\end{dependencies}
