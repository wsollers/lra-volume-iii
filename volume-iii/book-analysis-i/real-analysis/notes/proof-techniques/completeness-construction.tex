...
% ============================================================
%  \S{}2  Completeness as a Constructive Engine
%  volume-iii/book-analysis-i/real-analysis/notes/proof-techniques/
%  completeness-construction.tex
% ============================================================

\subsection{Completeness as a Constructive Engine}
...
\begin{tabular}{l l}
\toprule
\textbf{Tool}  & \textbf{What it supplies} \\
\midrule
\hyperref[prop:eps-char-sup]{$\varepsilon$-characterisation of $\sup$}
   & $\forall\varepsilon>0,\;\exists x\in S,\;x>\sup S-\varepsilon$
  \\[3pt]
\hyperref[thm:inductive-selection]{Inductive selection}
   & Monotone $(x_n)\subset S$ with $x_n\to\sup S$
  \\[3pt]
\hyperref[prop:monotone-approx-bounds]{Monotone Approximation}
   & Such a sequence exists for any nonempty bounded $S$
  \\[3pt]
\hyperref[thm:LUB-iff-MCT]{LUB $\Leftrightarrow$ MCT}
...
% -- \S{}2.1: The $\varepsilon$-characterisation -----------------------------
\subsection*{\texorpdfstring{The $\varepsilon$-Characterisation of Suprema and Infima}{The epsilon-Characterisation of Suprema and Infima}}
...
\end{proposition}
\LeanFormalizes{prop:eps-char-sup}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.CompletenessConstructionToolkit}{EpsCharSup}{pending}
...
\begin{remark}[Choosing a supremum proof strategy]
Most proofs that a candidate \(M\) is \(\sup S\) have two parts.  First prove
that \(M\) is an upper bound:
\[
  \forall x\in S,\qquad x\le M.
\]
Then prove sharpness, meaning that no smaller number is still an upper bound.
Several tools can prove this second part.

Use the definitional method when an arbitrary upper bound can be transported
cleanly through the construction.  Assume \(u\) is an upper bound for \(S\),
then prove \(M\le u\).  This is often the shortest route for translations,
positive dilations, and order-preserving changes of variables.  For example,
if \(u\) bounds \(A+c\), then \(u-c\) bounds \(A\), so
\(\sup A\le u-c\), and hence \(\sup A+c\le u\).

Use the epsilon characterization when it is easier to build elements close to
the proposed supremum.  Fix \(\varepsilon>0\), then find
\(x_\varepsilon\in S\) such that
\[
  M-\varepsilon < x_\varepsilon \le M.
\]
This is useful when the construction combines independent choices.  For
instance, to prove \(\sup(A+B)=\sup A+\sup B\), choose \(a\in A\) within
\(\varepsilon/2\) of \(\sup A\) and \(b\in B\) within \(\varepsilon/2\) of
\(\sup B\).  Then \(a+b\in A+B\) lies within \(\varepsilon\) of
\(\sup A+\sup B\).  Exact maximizers need not exist; near-maximizers are
enough.

Use comparison when another set already has a known supremum.  To prove
\(\sup S\le \sup T\), it is enough to show that \(\sup T\) is an upper bound
for \(S\), or to show that every element of \(S\) lies below some element of
\(T\).  Inclusion \(S\subseteq T\) is the basic example.

Use attainment when \(M\in S\).  If \(M\) is an upper bound and \(M\in S\),
then \(M=\max S=\sup S\), so no epsilon argument is needed.

For sums of functions with a shared input, the epsilon method proves only the
upper estimate in general:
\[
  \sup_x(f(x)+g(x))\le \sup_x f(x)+\sup_x g(x).
\]
Equality requires an additional reason that the two functions have compatible
near-maximizers.  For independent variables, such as
\(\sup_{x,y}(f(x)+g(y))\), the separate epsilon choices can usually be
combined.

The infimum versions reverse the inequalities: prove a lower bound first,
then prove greatestness by arbitrary lower bounds, epsilon approximation from
above, comparison, or attainment of a minimum.
\end{remark}

% -- \S{}2.2: Inductive selection --------------------------------
...
(\cref{thm:archimedean-property}), forcing $|s-x_n|\leq\tfrac{1}{n}\to 0$.
...
\end{proposition}
\LeanFormalizes{prop:monotone-approx-bounds}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.CompletenessConstructionToolkit}{MonotoneApproxBounds}{pending}
...
% -- \S{}2.4: LUB $\leftrightarrow$ MCT equivalence ------------------------------
\subsection*{\texorpdfstring{LUB $\Longleftrightarrow$ MCT: The Completeness Equivalence}{LUB Longleftrightarrow MCT: The Completeness Equivalence}}
\label{subsec:LUB-MCT}

\begin{remark*}[Interpretation]
The canonical definition is
\hyperref[def:least-upper-bound-property]{Least Upper Bound Property}. The
canonical equivalence theorem is
\hyperref[thm:lub-property-equivalent-monotone-convergence]{Least Upper Bound
Property Equivalent to Monotone Convergence}.

In abbreviated form, the equivalence says:
