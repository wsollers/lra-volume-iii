...
\[
  \operatorname{DifferentiableAt}(f',c,f''(c))
...
\input{volume-iii/book-analysis-iii/differentiation/notes/graph-reading/figure-second-derivative-tests}
...
\begin{remark*}[Standard quantified statement]
\[
  f'(c)=0\land f''(c)>0 \Rightarrow \operatorname{StrictRelativeMinimum}(f,c),
\]
and
\[
  f'(c)=0\land f''(c)<0 \Rightarrow \operatorname{StrictRelativeMaximum}(f,c).
\]
\end{remark*}
...
\begin{remark*}[Standard quantified statement]
\[
  \operatorname{InflectionPoint}(f,c)\land \operatorname{ContinuousAt}(f'',c)
  \Longrightarrow
  f''(c)=0.
\]
\end{remark*}
