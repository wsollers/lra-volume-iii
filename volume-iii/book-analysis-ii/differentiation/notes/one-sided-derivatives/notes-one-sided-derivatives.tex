...
A corner is a place where a curve has a left answer and a right answer and
refuses to choose. One-sided derivatives interrogate each side alone, and
their agreement criterion is the fastest way to certify many failures of
differentiability.
\end{remark*}

\begin{definitionbox}{Definition (Left-Hand Derivative)}
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:left-hand-derivative}{lra-lean}{LRA.VolumeIII.Analysis.Differentiation.OneSidedDerivatives}{LeftDerivative}{pending}
...
\operatorname{LeftDerivative}(L,f,c,\mathbb{R},\mathbb{R})
\;\Longleftrightarrow\;
...
\begin{aligned}
  f\colon A\to\mathbb{R},\\
\forall L \in \mathbb{R}\;\lnot\operatorname{LeftDerivative}(L,f,c,\mathbb{R},\mathbb{R}).
\end{aligned}
\]
...
\begin{description}
  \item[Exposition.]
  The failure is recorded by the negated formal condition.
...
\begin{definitionbox}{Definition (Right-Hand Derivative)}
...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:right-hand-derivative}{lra-lean}{LRA.VolumeIII.Analysis.Differentiation.OneSidedDerivatives}{RightDerivative}{pending}
...
\begin{aligned}
  f\colon A\to\mathbb{R},\\
\operatorname{RightDerivative}(L,f,c,\mathbb{R},\mathbb{R})
\;\Longleftrightarrow\;
...
\begin{aligned}
  f\colon A\to\mathbb{R},\\
\forall L \in \mathbb{R}\;\lnot\operatorname{RightDerivative}(L,f,c,\mathbb{R},\mathbb{R}).
\end{aligned}
\]
\end{remark*}
...
\begin{remark*}[Interpretation]
The right-hand derivative $f'_+(c)$ is the instantaneous rate of change
seen from inputs larger than $c$. It is the endpoint derivative available
at the left endpoint of a closed interval, and it is one half of the
two-sided derivative test at interior points.
\end{remark*}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:differentiability-and-one-sided-derivatives}{lra-lean}{LRA.VolumeIII.Analysis.Differentiation.OneSidedDerivatives}{DifferentiableIffOneSidedDerivativesAgree}{pending}

\begin{remark*}[Standard quantified statement]
\[
\operatorname{IsDifferentiable}(f,c,\mathbb{R},\mathbb{R})
\;\iff\;
\Bigl(\operatorname{LeftDerivative}(L,f,c,\mathbb{R},\mathbb{R}) \;\land\; \operatorname{RightDerivative}(L,f,c,\mathbb{R},\mathbb{R})\Bigr)
...
\begin{remark*}[Negated quantified statement]
\[
\lnot\operatorname{IsDifferentiable}(f,c,\mathbb{R},\mathbb{R})
\;\iff\;
\lnot\Bigl(\operatorname{LeftDerivative}(L,f,c,\mathbb{R},\mathbb{R}) \;\land\; \operatorname{RightDerivative}(L,f,c,\mathbb{R},\mathbb{R})\Bigr)
...
\begin{exposition}
\phantomsection\label{ex:abs-corner}
For \(f(x)=|x|\) at \(c=0\), the one-sided difference quotients are
\[
\begin{aligned}
  \frac{|x|-0}{x-0}
  =
...
\end{aligned}
\]
Thus \(f'_-(0)=-1\) and \(f'_+(0)=1\). The two values disagree, so the
two-sided derivative does not exist at \(0\), even though \(f\) is continuous
there.
\end{exposition}
