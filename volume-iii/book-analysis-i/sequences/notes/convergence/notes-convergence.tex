...
\forall \varepsilon > 0 \;\exists N \in \mathbb{N} \;\forall n \ge N
\;\bigl(
  \operatorname{IsCenteredOpenInterval}(I_\varepsilon(L),L,\varepsilon,\mathbb R)
  \wedge
  \operatorname{Within}(x_n,I_\varepsilon(L),\mathbb R)
\bigr).
\]
\end{remark*}
...
\[
\neg\operatorname{ConvergesTo}(x_n,L)
\;:=\;
\exists \varepsilon > 0 \;\forall N \in \mathbb{N} \;\exists n \ge N
...
centered $\varepsilon$-interval about $L$.
\end{remark*}
...
\begin{definitionbox}{Definition (Convergence --- Centered Interval Formulation)}
% ---------------------------------------------------------
% Definition: Convergence --- Centered Interval Formulation
% Label: def:convergent-sequence-neighborhood
% ---------------------------------------------------------
\begin{definition}[Convergence --- Centered Interval Formulation]
\label{def:convergent-sequence-neighborhood}
...
\begin{remark*}[Negation predicate reading]
\[
\neg\operatorname{ConvergesTo}(x_n,L,\mathbb R)
\;:=\;
\exists \varepsilon > 0 \;\forall N \in \mathbb{N} \;\exists n \ge N
\;\bigl(x_n \notin I_\varepsilon(L)\bigr).
\]
...
\begin{description}
  \item[Exposition.]
Failure of the centered interval formulation is exactly failure to trap a
tail in every centered interval about $L$. Different examples may escape by
oscillation, by monotone drift, or by unbounded growth, but the formal
failure is always the existence of one interval that every tail
...
\begin{description}
  \item[Exposition.]
\[
\underbrace{\exists \varepsilon > 0}_{\text{a fixed centered interval}}
\;\underbrace{\forall N \in \mathbb{N}}_{\text{no tail suffices}}
\;\underbrace{\exists n \ge N}_{\text{terms escape infinitely often}}
\;\underbrace{x_n \notin I_\varepsilon(L)}_{\text{outside every candidate interval}}.
\]
The sequence fails to converge to $L$ precisely when some centered interval
...
\begin{remark*}[Interpretation]
The centered interval formulation reframes convergence as a set-containment
condition rather than an inequality condition. The assertion $x_n \in
I_\varepsilon(L)$ is identical to $|x_n - L| < \varepsilon$; the two
definitions are logically equivalent and the choice between them is one
of language, not content. The local-set language is the natural
bridge to the topological formulation: in a general topological space,
open sets replace centered $\varepsilon$-intervals and the definition reads
identically with $I_\varepsilon(L)$ replaced by any open set $U$
...
are handled separately. Condition (d) is the centered interval
formulation: $x_n \in I_\varepsilon(L)$ is identical to conditions
...
\begin{itemize}
  \item \hyperref[def:convergent-sequence]{Convergence of a Sequence}
  \item \hyperref[def:convergent-sequence-neighborhood]{Convergence --- Centered Interval Formulation}
\end{itemize}
\end{dependencies}
