...
\begin{description}
  \item[Exposition.]
  The lemma can fail only if the hypothesis supplies a positive base and a
  natural-number exponent while the resulting power is not positive.

  \item[\operatorname{Positive}.]
  A counterexample must keep the base positive but make the power nonpositive.
  \[
    x>0
    \quad\text{and}\quad
    x^n\le 0.
...
\begin{description}
  \item[Exposition.]
  The lemma can fail only if the inputs satisfy the positive strict order
  hypothesis while the $n$th-power comparison reverses or collapses.

  \item[Order comparison.]
  A counterexample preserves $0<x<y$ but has $y^n$ no larger than $x^n$.
  \[
    0<x<y
    \quad\text{and}\quad
    y^n\le x^n.
