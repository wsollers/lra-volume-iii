% !TEX root = ../../../../../volume-iii-bounds-sequences-and-series.tex
% =========================================================
% Modulus: Algebra of Absolute Value
% =========================================================

\subsection*{Modulus}
\label{sec:modulus}
% ---------------------------------------------------------
% def:absolute-value
% ---------------------------------------------------------

\begin{definitionbox}{Definition (Absolute Value)}
\begin{definition}[Absolute Value]
\label{def:absolute-value}
Let $a \in \mathbb{R}$.  The \emph{absolute value} (or \emph{modulus}) of
$a$ is
\[
  \lvert a \rvert
  \;:=\;
  \begin{cases}
    a  & \text{if } a \ge 0, \\
    -a & \text{if } a < 0.
  \end{cases}
\]
Equivalently, $\lvert a \rvert := \max(a, -a)$.
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
  \lvert a \rvert
  \;=\;
  \max(a,\,-a)
  \;\coloneqq\;
  \begin{cases}
    a  & \text{if } a \ge 0, \\
    -a & \text{if } a < 0.
  \end{cases}
\]
\end{remark*}

\begin{remark*}[Interpretation]
The absolute value measures magnitude independent of sign.  On the real
line it equals the distance from $a$ to $0$: $\lvert a \rvert = d(a, 0)$
under the standard metric.  The two-branch definition partitions
$\mathbb{R}$ at $0$ and reflects negative values to positive ones.
\end{remark*}

\begin{dependencies}
\hyperref[def:real-ordered-field]{Ordered field},
\hyperref[thm:q-is-an-ordered-field]{$\mathbb{R}$ is an ordered field}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-nonneg
% ---------------------------------------------------------

\begin{theorem}[Absolute Value Is Nonnegative]
\label{thm:absolute-value-nonneg}
For all $a \in \mathbb{R}$,
\[
  \lvert a \rvert \ge 0.
\]
\hyperref[prf:absolute-value-nonneg]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-nonneg}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueNonneg}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\quad \lvert a \rvert \ge 0.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Magnitude is never negative; absolute value lands in the nonnegative part of the real line.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[prop:product-of-positives-is-positive-q]{squares are nonnegative},
\hyperref[def:real-ordered-field]{ordered field}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-zero-iff-zero
% ---------------------------------------------------------

\begin{theorem}[Absolute Value Zero Iff Zero]
\label{thm:absolute-value-zero-iff-zero}
For all $a \in \mathbb{R}$,
\[
  \lvert a \rvert = 0 \iff a = 0.
\]
\hyperref[prf:absolute-value-zero-iff-zero]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-zero-iff-zero}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueZeroIffZero}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\quad
  \lvert a \rvert = 0 \iff a = 0.
\]
\end{remark*}

\begin{remark*}[Interpretation]
The only real number with zero magnitude is the origin itself.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:absolute-value-nonneg]{absolute value is nonnegative},
\hyperref[def:real-ordered-field]{ordered field}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-self-or-neg
% ---------------------------------------------------------

\begin{theorem}[Absolute Value Equals Self or Negation]
\label{thm:absolute-value-self-or-neg}
For all $a \in \mathbb{R}$, either $\lvert a \rvert = a$ or
$\lvert a \rvert = -a$.
\hyperref[prf:absolute-value-self-or-neg]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-self-or-neg}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueSelfOrNeg}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\quad
  \lvert a \rvert = a \;\lor\; \lvert a \rvert = -a.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Absolute value chooses the representative of a number's sign pair that lies on the nonnegative side.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-symmetric
% ---------------------------------------------------------

\begin{theorem}[Absolute Value Is Symmetric]
\label{thm:absolute-value-symmetric}
For all $a \in \mathbb{R}$,
\[
  \lvert -a \rvert = \lvert a \rvert.
\]
\hyperref[prf:absolute-value-symmetric]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-symmetric}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueSymmetric}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\quad \lvert -a \rvert = \lvert a \rvert.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Changing sign does not change distance from zero.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
double negation.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-product
% ---------------------------------------------------------

\begin{theorem}[Absolute Value of a Product]
\label{thm:absolute-value-product}
For all $a, b \in \mathbb{R}$,
\[
  \lvert ab \rvert = \lvert a \rvert\,\lvert b \rvert.
\]
\hyperref[prf:absolute-value-product]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-product}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueProduct}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a, b \in \mathbb{R},\quad
  \lvert ab \rvert = \lvert a \rvert\,\lvert b \rvert.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
|ab|=|a|\,|b|.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Magnitude is multiplicative: the size of a product is the product of the sizes.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
negation of product,
product of negatives,
\hyperref[prop:product-of-positives-is-positive-q]{squares are nonnegative}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-quotient
% ---------------------------------------------------------

\begin{theorem}[Absolute Value of a Quotient]
\label{thm:absolute-value-quotient}
For all $a \in \mathbb{R}$ and $b \in \mathbb{R}$ with $b \ne 0$,
\[
  \left\lvert \frac{a}{b} \right\rvert
  = \frac{\lvert a \rvert}{\lvert b \rvert}.
\]
\hyperref[prf:absolute-value-quotient]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-quotient}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueQuotient}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\;
  \forall b \in \mathbb{R} \setminus \{0\},\quad
  \left\lvert \frac{a}{b} \right\rvert
  = \frac{\lvert a \rvert}{\lvert b \rvert}.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
b\ne0\Rightarrow \left|\frac ab\right|=\frac{|a|}{|b|}.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Taking magnitudes commutes with division when the denominator is nonzero.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:absolute-value-product]{absolute value of a product},
\hyperref[thm:absolute-value-zero-iff-zero]{absolute value zero iff zero},
\hyperref[cor:uniqueness-multiplicative-inverse-on-q]{uniqueness of
multiplicative inverse}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-bounds
% ---------------------------------------------------------

\begin{theorem}[Bound by Modulus]
\label{thm:absolute-value-bounds}
For all $a \in \mathbb{R}$,
\[
  -\lvert a \rvert \le a \le \lvert a \rvert.
\]
\hyperref[prf:absolute-value-bounds]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-bounds}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueBounds}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\quad
  -\lvert a \rvert \le a \le \lvert a \rvert.
\]
\end{remark*}

\begin{remark*}[Interpretation]
Every real number sits between its negative magnitude and its positive magnitude.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:absolute-value-nonneg]{absolute value is nonnegative},
\hyperref[def:real-ordered-field]{ordered field}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-le-iff
% ---------------------------------------------------------

\begin{theorembox}{Theorem (Nonstrict Modulus Inequality)}
\begin{theorem}[Nonstrict Modulus Inequality]
\label{thm:absolute-value-le-iff}
For all $a \in \mathbb{R}$ and $r \ge 0$,
\[
  \lvert a \rvert \le r \iff -r \le a \le r.
\]
\hyperref[prf:absolute-value-le-iff]{\textit{Go to proof.}}
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:absolute-value-le-iff}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueLeIff}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\;
  \forall r \in \mathbb{R},\;
  r \ge 0 \;\Rightarrow\;
  \bigl(\lvert a \rvert \le r \iff -r \le a \le r\bigr).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
r\ge0\Rightarrow \bigl(|a|\le r\Longleftrightarrow -r\le a\le r\bigr).
\]
\end{remark*}

\begin{remark*}[Interpretation]
This biconditional is the primary tool for replacing a modulus inequality
with a pair of simultaneous linear inequalities.  It underlies the
$\varepsilon$-$\delta$ manipulations throughout analysis: the condition
$\lvert x - a \rvert \le \varepsilon$ is equivalent to
$a - \varepsilon \le x \le a + \varepsilon$.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:absolute-value-bounds]{bound by modulus},
\hyperref[def:real-ordered-field]{ordered field}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-lt-iff
% ---------------------------------------------------------

\begin{theorem}[Strict Modulus Inequality]
\label{thm:absolute-value-lt-iff}
For all $a \in \mathbb{R}$ and $r > 0$,
\[
  \lvert a \rvert < r \iff -r < a < r.
\]
\hyperref[prf:absolute-value-lt-iff]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-lt-iff}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueLtIff}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a \in \mathbb{R},\;
  \forall r \in \mathbb{R},\;
  r > 0 \;\Rightarrow\;
  \bigl(\lvert a \rvert < r \iff -r < a < r\bigr).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
r>0\Rightarrow \bigl(|a|<r\Longleftrightarrow -r<a<r\bigr).
\]
\end{remark*}

\begin{remark*}[Interpretation]
Strict modulus inequalities are exactly strict two-sided bounds. This is the
form used when a punctured or ordinary neighbourhood is unpacked into
ordinary order inequalities.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:absolute-value-le-iff]{nonstrict modulus inequality},
\hyperref[def:real-ordered-field]{ordered field}.
\end{dependencies}

% ---------------------------------------------------------
% Triangle inequality
% ---------------------------------------------------------

\begin{restatementbox}{Restatement (Triangle Inequality)}
For all $a, b \in \mathbb{R}$,
\[
  \lvert a + b \rvert \le \lvert a \rvert + \lvert b \rvert.
\]

The triangle inequality is the foundational estimate of analysis.  Every
convergence argument, continuity proof, and metric-space result that
requires bounding a sum reduces, at some step, to this inequality.  The
name reflects its geometric meaning: the length of one side of a triangle
does not exceed the sum of the other two.
\end{restatementbox}

% ---------------------------------------------------------
% thm:reverse-triangle-inequality
% ---------------------------------------------------------

\begin{theorem}[Reverse Triangle Inequality]
\label{thm:reverse-triangle-inequality}
For all $a, b \in \mathbb{R}$,
\[
  \lvert\, \lvert a \rvert - \lvert b \rvert \,\rvert
  \le \lvert a - b \rvert.
\]
\hyperref[prf:reverse-triangle-inequality]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:reverse-triangle-inequality}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{ReverseTriangleInequality}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall a, b \in \mathbb{R},\quad
  \lvert\, \lvert a \rvert - \lvert b \rvert \,\rvert
  \le \lvert a - b \rvert.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\bigl||a|-|b|\bigr|\le |a-b|.
\]
\end{remark*}

\begin{remark*}[Interpretation]
The reverse triangle inequality provides a lower bound on $\lvert a - b
\rvert$ in terms of the difference of moduli.  It is used to bound the
modulus of a difference from below, particularly when proving that a
function is not uniformly continuous or that a sequence does not converge.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:triangle-inequality]{triangle inequality},
\hyperref[thm:absolute-value-symmetric]{absolute value is symmetric}.
\end{dependencies}

% ---------------------------------------------------------
% thm:absolute-value-sum-bound
% ---------------------------------------------------------

\begin{theorem}[Generalised Triangle Inequality]
\label{thm:absolute-value-sum-bound}
For all $n \in \mathbb{N}$ and $a_1, \ldots, a_n \in \mathbb{R}$,
\[
  \left\lvert \sum_{k=1}^{n} a_k \right\rvert
  \le \sum_{k=1}^{n} \lvert a_k \rvert.
\]
\hyperref[prf:absolute-value-sum-bound]{\textit{Go to proof.}}
\end{theorem}
\LeanFormalizes{thm:absolute-value-sum-bound}{lra-lean}{LRA.VolumeIII.Analysis.RealAnalysis.Modulus}{AbsoluteValueSumBound}{pending}



\begin{remark*}[Standard quantified statement]
\[
  \forall n \in \mathbb{N},\;
  \forall a_1, \ldots, a_n \in \mathbb{R},\quad
  \left\lvert \sum_{k=1}^{n} a_k \right\rvert
  \le \sum_{k=1}^{n} \lvert a_k \rvert.
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\left|\sum_{k=1}^n a_k\right|\le \sum_{k=1}^n |a_k|.
\]
\end{remark*}

\begin{remark*}[Interpretation]
The magnitude of a finite sum is controlled by summing the magnitudes of its terms.
\end{remark*}

\begin{dependencies}
\hyperref[def:absolute-value]{Absolute value},
\hyperref[thm:triangle-inequality]{triangle inequality}.
\end{dependencies}
